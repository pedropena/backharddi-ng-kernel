...
void help(void)
{
    ...
    fg = con_fg(f,fg);
#if defined(USE_LZO1X_1_15) && defined(USE_LZO1X_999)
    con_fprintf(f,"  -1     compress faster                   -9    compress better\n");
#elif defined(USE_LZO1X_1_15)
    con_fprintf(f,"  -1     compress faster\n");
#endif
    con_fprintf(f,"  -d     decompress                        -x    extract (same as -dPp)\n");
    con_fprintf(f,"  -l     list compressed file              -I    display system information\n");
    con_fprintf(f,"  -t     test compressed file              -V    display version number\n");
    con_fprintf(f,"  -h     give this help                    -L    display software license\n");
...
void version(void)
{
    ...
    for (i = 0, qq = __lzo_copyright; i < sizeof(pp)-1 && *qq; i++, qq++)
        pp[i] = (char) *qq;
    pp[i] = 0;
