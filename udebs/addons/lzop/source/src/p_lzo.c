...
#define LSIZE(size) \
    ALIGN_UP((size) + (ALIGN_SIZE > 1 ? ALIGN_SIZE - 1 : 0), sizeof(long))
#define HEAP_ALLOC(var,size) \
    long __LZO_MMODEL var [ LSIZE(size) / sizeof(long) ]
static HEAP_ALLOC(heap_block1, BLOCK_SIZE);
static HEAP_ALLOC(heap_block2, MAX_COMPRESSED_SIZE(BLOCK_SIZE));
static HEAP_ALLOC(heap_wrkmem, WRK_LEN);
#endif
