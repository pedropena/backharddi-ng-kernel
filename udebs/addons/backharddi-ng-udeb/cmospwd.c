...
void table2val(const int *table, unsigned char *value, const int lng, const uint8_t *data_src);
...
void generic_basic(const int position, const int lng, const int meth_parcourt,const int algo,const int val_stop,const int mode_aff, const uint8_t *data_src);
...
uint8_t parity_test(uint8_t);
uint16_t rol(uint16_t);
uint8_t rcl8(uint8_t);
uint8_t rcl8n(uint8_t, int);
...
uint8_t rcl8n(uint8_t num,int n)
{
  int i;
  uint8_t res=num;
  for(i=0;i<n;i++)
...
void generic_table(const int *table, const int lng, const int algo,const int val_stop,const int mode_aff, const uint8_t *data_src);
...
void generic_basic(const int position, const int lng, const int meth_parcourt,const int algo,const int val_stop,const int mode_aff, const uint8_t *data_src)
{
  ...
  generic_table(table,lng,algo,val_stop,mode_aff,data_src);
}
...
#endif
  table2val(table,value,lng,data_src);
  switch(algo)
  {
    case ALGO_AMI_F0:
    case ALGO_AMI:
...
{
  printf(m_acer);
  generic_basic(0x27, 7, METH_PARC_NORMAL,ALGO_ACER,0,AFF_SCAN,cmos);
  generic_basic(0x100, 7, METH_PARC_NORMAL, ALGO_ACER, 0,AFF_ASCII,cmos);
}

void ami_old()				/* AMI */
{
  printf(m_ami_old);
  generic_basic(0x37, 1+6, METH_PARC_NORMAL,ALGO_AMI_F0,0,AFF_ASCII,cmos);
}

void ami_winbios()
{
  printf(m_ami_winbios);
  generic_basic(0x37, 1+6, METH_PARC_NORMAL,ALGO_AMI,0,AFF_SCAN,cmos);
}

void ami_winbios2()
{
  printf(m_ami_winbios2);
  generic_basic(0x37, 1+6, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
  generic_basic(0x4B, 1+6, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
  /* setup, Added by Tompa Lorand, 01-apr-2003 */
  generic_basic(0x5f, 1+6, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
...
void ami_unk()
{
  ...
  printf(m_ami_unk);
  generic_basic(0x4d, 1+7, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
  generic_basic(0x54, 1+7, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
  // Rajabaz <unknown__14@hotmail.com>
  generic_basic(0x4c, 1+7, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
  generic_basic(0x53, 1+7, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
  /* Bidouilleurs LBCU <4c.42.43.55@gmail.com> */
  generic_basic(0x50, 1+7, METH_PARC_NORMAL,ALGO_AMI_80,0,AFF_SCAN,cmos);
}

void samsung()
{
  printf(m_samsung_P25);
  generic_basic(0xE3,7,METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0xF0,7,METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0xF8,7,METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}

void sony_vaio()
{
  printf(m_Sony_Vaio);
  generic_basic(0x00,7,METH_PARC_NORMAL,ALGO_ACER,0,AFF_ASCII,cmos);
  putchar(' ');
  generic_basic(0x07,7,METH_PARC_NORMAL,ALGO_ACER,0,AFF_ASCII,cmos);
#ifdef __linux__
  {
    FILE *fb;
    ...
    fclose(fb);
    putchar(' ');
    generic_basic(0x00,7,METH_PARC_NORMAL,ALGO_ACER,0,AFF_ASCII,eeprom);
    putchar(' ');
    generic_basic(0x07,7,METH_PARC_NORMAL,ALGO_ACER,0,AFF_ASCII,eeprom);
  }
#endif
}
...
{
  printf(m_zenith_ami);
  generic_basic(0x38+1, 7, METH_PARC_NORMAL,ALGO_UNKNOW,0,AFF_ASCII,cmos);
  putchar(' ');
  generic_basic(0x40+1, 7, METH_PARC_NORMAL,ALGO_UNKNOW,0,AFF_ASCII,cmos);
}
...
void award()
{
  ...
  printf(m_award);
  generic_crc(ALGO_AWARD,0x63,cmos); /* putamadre7@hotmail.com */
  generic_crc(ALGO_AWARD,0x64,cmos); /* jedi@ukgateway.net */
  generic_crc(ALGO_AWARD,0x5D,cmos); /* Setup YOGESH M <my@myw.ltindia.com> */
...
*/
  ...
  generic_crc(ALGO_AWARD,0x4E,cmos); /*  Lewis DH <lewisdh@hotmail.com> */
  generic_crc(ALGO_AWARD,0x71,cmos); /*  Lewis DH <lewisdh@hotmail.com> */
}
...
void award6()
{
  ...
  generic_basic(0xE0, 8, METH_PARC_NORMAL,ALGO_AWARD6,0,AFF_ASCII,cmos);
  /* Ai-Nung Wang <wang@math.ntu.edu.tw>, october 2003 */
  generic_basic(0xB0, 8, METH_PARC_NORMAL,ALGO_AWARD6,0,AFF_ASCII,cmos);
  /* Jason Gorski <jgorski@gmail.com>, january 2005 */
  generic_basic(0x90, 8, METH_PARC_NORMAL, ALGO_AWARD6, 0, AFF_ASCII,cmos);
}
...
{
  printf(m_compaq);
  generic_basic(0x38, 8, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}

void compaq2()
{
  printf(m_compaq2);
  generic_basic(0x51, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos); /* setup */
  generic_basic(0x38, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}
...
void compaq3()
{
   ...
   */
  generic_basic(0x37, 8, METH_PARC_NORMAL,ALGO_COMPAQ,0,AFF_SCAN,cmos);
   /* - Quattrocchi Stefano <S.Quattrocchi@tecnimont.it>
    *   Compaq DeskPro EP Serie 6350/6.4 EA2, May 2001
    */
  generic_basic(0x77, 8, METH_PARC_NORMAL,ALGO_COMPAQ,0,AFF_SCAN,cmos);
}
...
{
  printf(m_ibm);
  generic_basic(0x48, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0x38, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}
...
void ibm_thinkpad()
{
  ...
  generic_basic(0x38, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);	/* pwd boot */
}

void ibm_thinkpad2()
{
  printf(m_ibm_thinkpad_x20_570_t20);
  generic_basic(0x338, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0x3B8, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  printf(m_ibm_thinkpad_560x);
  generic_basic(0x38, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0x40, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
...
void packardbell()				/* PACKARD BELL */
{
  ...
  generic_basic(0x38, 1+7, METH_PARC_NORMAL,ALGO_PACKARD,0xFF,AFF_ASCII,cmos);
  putchar(' ');
  generic_basic(0x40, 1+7, METH_PARC_NORMAL,ALGO_PACKARD,0xFF,AFF_ASCII,cmos);
}
...
void phoenix()				/* PHOENIX */
{
  ...
  if (crc==cmos[tbl_phoenix[7]])
    ...
    printf("CRC pwd err");
  generic_table(tbl_phoenix, 7, ALGO_PHOENIX_XOR,0,AFF_ASCII,cmos);
}

void phoenix2()
{
  printf(m_phoenix2);
  generic_basic(0x50, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos); /* setup */
  generic_basic(0x48, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}
...
{ /* Phoenix Bios V1.10 A03 / Dell GXi */
  ...
  generic_basic(0x1D, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos); /* setup */
  generic_basic(0x38, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}
...
{
  printf(m_phoenix4);
  generic_basic(0x35, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos); /* user */
}
...
{
  printf(m_phoenix5);
  generic_basic(0x35, 7, METH_PARC_NORMAL,ALGO_PHOENIX_DEC,0,AFF_SCAN,cmos);
}
...
void phoenix6()
{
  ...
  if(((cmos[0x60]==0)||(cmos[0x60]==1))&&(cmos[0x61]<=7))
    generic_basic(0x62, cmos[0x61], METH_PARC_NORMAL,ALGO_PHOENIX_ADD,0xFF,AFF_ASCII,cmos);
  else
    printf("err");
  /* CRC 32 en 7E
   * 3B-3D => 6A-6C
...
void phoenix405()
{
  ...
  printf(m_phoenix405);
  generic_table(tbl, 8, ALGO_NONE,0,AFF_SCAN,cmos);
  generic_table(tbl2, 8, ALGO_NONE,0,AFF_SCAN,cmos);
}

void phoenix406()
{
  printf(m_phoenix406);
  generic_basic(0x45, 8, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}

void dtk()
{
  printf(m_dtk);
  generic_basic(0x38,4,METH_PARC_NORMAL,ALGO_DTK,0,AFF_ASCII,cmos);
  generic_basic(0x3B,6,METH_PARC_NORMAL,ALGO_DTK,0,AFF_ASCII,cmos);
}
...
void generic_compaq(unsigned char *value, const int lng,const unsigned char value_stop)
{
   ...
   *   Compaq 5200, August 1998
   * - Quattrocchi Stefano <S.Quattrocchi@tecnimont.it>
   * - Zoulou Yankee <zyankee@ifrance.com>
   *   Compaq DeskPro EP Serie 6350/6.4 EA2, May 2001
   */
...
{	/* Gateway Solo */
  printf(m_gateway_ph);
  generic_basic(0x40, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0x47, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}

void phoenixa08()
{
  printf(m_phoenixa08);
  generic_basic(0x23, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
  generic_basic(0x42, 7, METH_PARC_NORMAL,ALGO_NONE,0,AFF_SCAN,cmos);
}
...
int load_backup(const char* name)
{
  ...
  fclose(fb);
  if((taille==64)||(taille==128)||(taille==256)||(taille==384))
  {
...
int main(int argc, char *argv[])
{
  ...
  memset(cmos, 0, sizeof(cmos));
  {
    ...
    for(i=1;i<argc;i++)
    {
      if(mode==MODE_LOAD || mode==MODE_RESTORE || mode==MODE_RESTORE_FORCE)
      {
	if((cmos_size=load_backup(argv[i]))<0)
	  return 1;
      }
      else if((arg_save_filename==-1)&&(mode==MODE_SAVE))
      {
	arg_save_filename=i;
      }
